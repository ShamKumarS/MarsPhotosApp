...
class MockMarsPhotosAPI: MarsPhotosServiceProtocol {
...
"""
    ...
    func getPhotos(with category: String, at pageNumber: Int, completion: @escaping (MarsPhotosResult) -> Void) {
        switch loadState {
            ...
            case .empty:
                completion(.success(MarsPhotos(photos: [])))
            default:
                do {
                    let jsonData = Data(testJSON.utf8)
                    let decoder = JSONDecoder()
                    let response = try decoder.decode(MarsPhotos.self, from: jsonData)
